...
final class MCEFJcefInstallationValidator {
    ...
    private static final Set<String> IDENTITY_KEYS = Set.of("archive_root", "java_cef_commit", "target");

    private MCEFJcefInstallationValidator() {
    }
    ...
    static boolean isReusable(Path installation, MCEFPlatform platform, String expectedCommit) {
        try {
    ...
    static void validateExtracted(Path installation, MCEFPlatform platform, String expectedCommit) throws IOException {
        validate(installation, platform, expectedCommit, false);
    }

    static void validateCompleted(Path installation, MCEFPlatform platform, String expectedCommit) throws IOException {
        validate(installation, platform, expectedCommit, true);
    }

    static void writeCompleteMarker(Path installation, MCEFPlatform platform, String expectedCommit) throws IOException {
    ...
    private static void validate(Path installation, MCEFPlatform platform, String expectedCommit, boolean requireComplete) throws IOException {
        String normalizedCommit = normalizeCommit(expectedCommit);
    ...
    private static void validateManifestIdentity(Path manifestPath, MCEFPlatform platform, String expectedCommit) throws IOException {
        ...
        try (JsonReader reader = new JsonReader(new StringReader(manifest))) {
            ...
            while (reader.hasNext()) {
                ...
                switch (key) {
                    case "archive_root" -> archiveRoot = readJsonString(reader, key);
                    case "java_cef_commit" -> commit = readJsonString(reader, key);
                    case "target" -> target = readJsonString(reader, key);
    ...
    private static List<String> requiredFiles(MCEFPlatform platform) {
        ...
        if (platform.isWindows()) {
            ...
            required.addAll(List.of("libjcef.so", "libcef.so", "jcef_helper", "chrome-sandbox", "icudtl.dat", "locales/en-US.pak"));
        } else {
            ...
            for (String suffix : List.of("", " (Alerts)", " (GPU)", " (Plugin)", " (Renderer)")) {
    ...
    private static String readSmallUtf8(Path path, int maximumBytes, String description) throws IOException {
        ...
        try {
            CharBuffer decoded = StandardCharsets.UTF_8.newDecoder().onMalformedInput(CodingErrorAction.REPORT).onUnmappableCharacter(CodingErrorAction.REPORT).decode(bytes.flip());
    ...
    private static String markerContents(MCEFPlatform platform, String commit) {
