...
final class MCEFJcefInstaller implements AutoCloseable {
    ...
    private static final int MAX_CLEANUP_ENTRIES = MCEFSecureArchiveExtractor.MAX_EXTRACTED_FILESYSTEM_ENTRIES + 192;
    ...
    MCEFJcefInstaller(Path librariesDirectory, MCEFPlatform platform, String javaCefCommit, Consumer<IOException> cleanupWarning) throws IOException {
        this.platform = Objects.requireNonNull(platform, "MCEF platform must not be null");
        this.javaCefCommit = MCEFJcefInstallationValidator.normalizeCommit(javaCefCommit);
        this.cleanupWarning = Objects.requireNonNull(cleanupWarning, "JCEF cleanup warning handler must not be null");

        Path configuredLibrariesDirectory = Objects.requireNonNull(librariesDirectory, "MCEF libraries directory must not be null").toAbsolutePath().normalize();
    ...
    Path findReusableInstallation() {
        requireOpenAndOwner();
        return MCEFJcefInstallationValidator.isReusable(installationDirectory, platform, javaCefCommit) ? installationDirectory : null;
    }
    ...
    Path publish() throws IOException {
        ...
        MCEFJcefInstallationValidator.validateExtracted(stagedInstallation, platform, javaCefCommit);
        ...
        MCEFJcefInstallationValidator.writeCompleteMarker(stagedInstallation, platform, javaCefCommit);
        forceDirectoryBestEffort(stagedInstallation);

        if (Files.exists(installationDirectory, LinkOption.NOFOLLOW_LINKS)) {
            if (MCEFJcefInstallationValidator.isReusable(installationDirectory, platform, javaCefCommit)) {
        ...
        try {
            ...
            throw new IOException("JCEF cache publication requires an atomic move on the mcef-libraries filesystem", failure);
        ...
        MCEFJcefInstallationValidator.validateCompleted(installationDirectory, platform, javaCefCommit);
        discardPreparedBestEffort();
        return installationDirectory;
    }
