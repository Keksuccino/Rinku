...
public class RinkuBrowser extends CefBrowserOsr {
    ...
    /**
     * Convenience method to get the resource location for this browser's texture.
     * This can be bound for 1.19.2's immediate GUI rendering methods.
     *
     * @return The resource location for this browser's texture, or null if not initialized
     */
    ...
    private void onPaintRenderThread(boolean popup, Rectangle[] dirtyRects, ByteBuffer buffer, int width, int height, Rectangle popupRect, boolean showPopupSnapshot, long popupStateGeneration, boolean forceFullUpload) {
        // CEF callbacks run inside Minecraft's frame and may interrupt Sodium/Iris-owned GL state. The guard is
        // deliberately outside the full multi-rectangle operation so state is queried and restored exactly once.
        try (RinkuGlStateGuard ignored = RinkuGlStateGuard.capture()) {
            onPaintRenderThreadGuarded(popup, dirtyRects, buffer, width, height, popupRect, showPopupSnapshot, popupStateGeneration, forceFullUpload);
        }
    }

    private void onPaintRenderThreadGuarded(boolean popup, Rectangle[] dirtyRects, ByteBuffer buffer, int width, int height, Rectangle popupRect, boolean showPopupSnapshot, long popupStateGeneration, boolean forceFullUpload) {
        if (!popup) {
