...
dependencies {
    modImplementation "de.keksuccino:rinku-fabric:3.0.0-1.19.2"
}
```
...
dependencies {
    implementation fg.deobf("de.keksuccino:rinku-forge:3.0.0-1.19.2")
}
```

ForgeGradle remaps the published Forge artifact through `fg.deobf`. Replace the Rinku and Minecraft version as required.

Minecraft 1.19.2 consumers can draw a ready browser through `RinkuBrowserTextureBlitter.blit(...)`. The helper handles CEF's premultiplied transparent pixels and restores the legacy GUI blend state after every draw.
...
Use Java 17 for builds and development runs.
