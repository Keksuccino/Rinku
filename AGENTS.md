...
- This project is "Rinku", a Minecraft Java 1.19.2 mod using the MultiLoader layout with shared logic under `common` and loader-specific wrappers under `fabric` and `forge`.
...
- Place shared Mixins under `common/src/main/java/de/keksuccino/rinku/mixins` and loader-only Mixins under the corresponding loader package.
...
- Cluster related injections together (for example, all `setScreen` hooks in `MixinMinecraft`, which owns that method in 1.19.2) and keep helper wrappers private unless a wider contract is required.
...
- Minecraft 1.19.2 uses the legacy immediate `PoseStack`/`Screen.render` GUI API and raw OpenGL texture uploads. Preserve the caller's texture binding, pixel-unpack buffer, and every changed unpack parameter around browser uploads. CEF transparent OSR pixels are premultiplied, so transparent GUI draws must use `ONE, ONE_MINUS_SRC_ALPHA` blending and restore Minecraft's default blend function plus blend-disabled screen postcondition afterward.
