...
- This project is "Rinku" for Minecraft Java 1.20.1. It uses the MultiLoader layout with shared logic under `common` and loader-specific wrappers under `fabric` and `forge`.
...
- After compiling and normal tests succeeded, run the client of both `fabric` and `forge` modules via their client launch tasks, and check the log output, to see if stuff like Mixin injects succeeded, but make sure to CLOSE THE CLIENT WINDOWS after. Run Fabric both with its default Sodium/Iris development dependencies and with `-Prinku.disableRenderingCompatMods=true`. Only run clients when it makes sense to run them, which means when compiling is not enough to catch all possible issues.
