...
- After compiling and normal tests succeeded, run the client of both `fabric` and `forge` modules via their normal client launch tasks, and check the log output, to see if stuff like Mixin injects succeeded, but make sure to CLOSE THE CLIENT WINDOWS after. Only run clients when it makes sense to run them, which means when compiling is not enough to catch all possible issues.
