...
- This project is "Rinku", which is a Minecraft Java 26.1.2 mod (the version number is not a typo). It uses the MultiLoader layout with shared logic under `common` and loader-specific wrappers under `fabric` and `neoforge`.
...
- Sources for Sodium, Sodium Extra, and Iris, when available, are in `/Volumes/STUFF/CODING/WORKSPACES/Java/Minecraft Mods/.MINECRAFT_SOURCES/26.1.2/libraries/`.
